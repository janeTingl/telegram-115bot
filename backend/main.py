...
def reload_services(config):
    ...
    if ol.get("url"):
        drive_openlist.init_config(ol["url"], ol.get("username"), ol.get("password"))
...
async def lifespan(app: FastAPI):
    ...
    if tg.get("botToken"):
        asyncio.create_task(run_bot(tg["botToken"]))
...
async def get_trending_wallpapers():
    ...
    try:
        ...
        images = [f"https://image.tmdb.org/t/p/original{item.backdrop_path}" for item in results if hasattr(item, 'backdrop_path')][:8]
...
async def dl_115(d: Dict[str, Any]): return drive_115.add_offline_task(d.get("urls", []), d.get("cid", "0"))

# --- 123 & OpenList 接口 ---
@app.get("/api/123/files")
async def list_123(parent_id: str="0"): return await drive_123.get_file_list(parent_id)

@app.post("/api/trigger/123_offline")
async def dl_123(d: Dict[str, Any]): return await drive_123.add_offline_task(d.get("url"), d.get("filename"))

@app.get("/api/openlist/files")
async def list_ol(path: str="/"): return await drive_openlist.get_file_list(path)
